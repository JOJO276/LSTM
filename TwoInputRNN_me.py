...
def generate_data(seq,n):
    X = []
    # 序列的第i项和后面的TIMESTEPS-1项合在一起作为输入
    for i in range(n, len(seq) - n -1):
        X.append(seq[i -n: i +2])
...
def lstm_model(X, y):
    ...
    fw_cell = tf.nn.rnn_cell.MultiRNNCell([lstm_fw_cell] * NUM_LAYERS)
    bw_cell = tf.nn.rnn_cell.MultiRNNCell([lstm_bw_cell] * NUM_LAYERS)
    try:
        outputs, _, _ = tf.nn.bidirectional_rnn(fw_cell, bw_cell, x_,
                                              dtype=tf.float32)
    except Exception: # Old TensorFlow version only returns outputs not states
        outputs = tf.nn.bidirectional_rnn(fw_cell, bw_cell, x_,
                                        dtype=tf.float32)
    ...
    #loss = []
    # 平方差损失函数
    prediction, loss = learn.models.linear_regression(outputs, y)
...
def main():
    ...
    ####for abs
            train_X_abs = generate_data(complex_x[0:TRAINING_EXAMPLES + TIMESTEPS - 1], int(TIMESTEPS / 2))
            train_y_abs = complex_y[int(TIMESTEPS / 2):TRAINING_EXAMPLES + int(TIMESTEPS / 2) - 1]
            test_X_abs = generate_data(
                complex_x[TRAINING_EXAMPLES:TESTING_EXAMPLES + TRAINING_EXAMPLES + TIMESTEPS - 1],
                int(TIMESTEPS / 2))
            test_y_abs = complex_y[
                         TRAINING_EXAMPLES + int(TIMESTEPS / 2):TESTING_EXAMPLES + TRAINING_EXAMPLES + int(
                             TIMESTEPS / 2) - 1]
